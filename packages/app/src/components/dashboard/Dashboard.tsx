...
import { getSales } from "@/_lib/actions/dashboard";

export const Dashboard = ({ data }: any) => {

    const [loading, setLoading]: any = useState(false);
    const [response, setResponse]: any = useState(data);

    const handleClick = async () => {
        setLoading(true); // Mostrar estado de carga
        try {
            const result: any = await getSales(); // Llama a la acción del servidor
        ...
        <div className="container mx-auto p-4">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div className="bg-gray-800 p-6 rounded-lg shadow-md flex flex-col">
                    <h3 className="text-lg font-semibold mb-4">Saldo disponible para uso</h3>
                    <p className="text-4xl font-bold text-green-500 mb-4">170,00 US$</p>
                    <button onClick={() => handleClick()} className="bg-white text-purple-600 font-semibold py-2 px-4 rounded-lg mb-2 shadow-sm hover:bg-gray-100 transition">
                        Retirar fondos
                    </button>
                ...
                <div className="grid grid-cols-1 gap-4">
                    <Card title="Ganancias hasta la fecha" amount="272,00 US$" color="text-green-500" />
                    <Card title="Pagos en proceso de liquidación" amount="300,80 US$" color="text-yellow-500" />
                </div>

                {/* Tercera columna */}
                <div className="grid grid-cols-1 gap-4">
                    <Card title="Gastos hasta la fecha" amount="62,00 US$" color="text-red-400" />
                    <Card title="Retirados hasta la fecha" amount="208,00 US$" color="text-red-400" />
                </div>
            </div>
                ...
                <>
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-1 mt-4">
                        ...
                        <div className="bg-gray-600">
                            <PieChartDonut chartData={response.chartDataPie} chartConfig={response.chartConfigPie} />
                        </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 gap-4 mt-4">
                        <div className="bg-gray-700 pr-4">
                            <BarChartMultiple chartData={response.chartData} chartConfig={response.chartConfig} />
                        </div>
                    </div>
                </>
            }
